...
pub fn resolve_signal_timeout() -> Duration {
    signal_timeout_from(
        std::env::var("WIRKEN_SIGNAL_APPROVAL_TIMEOUT_S")
            .ok()
            .as_deref(),
    )
}
...
fn signal_timeout_from(raw: Option<&str>) -> Duration {
...
impl SignalApprovalGate {
    pub fn new(
        ...
        Self::with_timeout(queue, approvers, outbound, resolve_signal_timeout())
    }

    /// Construct with an explicit timeout instead of the value
    /// [`resolve_signal_timeout`] reads from the environment. Lets a test drive
...
mod tests {
    ...
    async fn timeout_path_forgets_queue_entry() {
        ...
        outbound.register("signal", writer);
        let gate = SignalApprovalGate::with_timeout(
            queue.clone(),
            approvers,
            outbound,
