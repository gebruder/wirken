...
- **Egress is not fully contained.** Sandboxed `exec` is bounded (no networking by default, a policed CONNECT proxy when a channel is granted egress), but MCP server children and the LLM client are not: both open their own outbound connections and need a network namespace, a restricted-egress container, or OS firewall rules. The sandbox egress tier also requires a runtime whose bridge gateway is a host interface, so rootless runtimes get `none` only. See [docs/egress.md](docs/egress.md).
