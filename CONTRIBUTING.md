...
**Tests that write a process-global serialise on it.** Three tests set, read and removed the same environment variable, under a comment saying the interleaving could not happen. The harness runs a binary's tests on parallel threads; one test read the value its sibling had just written and failed, once, in the run that mattered. A test that passes alone and passes three times in a row is not shown deterministic; the writers of a shared global take a lock.
