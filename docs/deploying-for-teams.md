...
- **Wirken does not bundle a content DLP scanner.** The prompt-injection detector flags inbound text for role-switching attempts, instruction overrides, base64-encoded commands, tool-call injection, and system prompt extraction, and writes the flag to the audit log. Inbound message content and outbound responses are not pattern-scanned by wirken itself for secrets, PII, or policy violations. Operators who need that scanning wire an external process as a `HookType::Egress` hook: it receives each tool output before it enters the LLM conversation and can `Allow`, `Replace`, or `Refuse` under an Ed25519-authenticated handshake; the audit chain records the redaction as a `tool_output_redacted` row with the original bytes' sha256 only. See [enforcement-model.md](enforcement-model.md) and [external-consumers.md](external-consumers.md).
