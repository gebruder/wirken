...
- **Tool-call preflight at run start.** Before any target-touching work, the runner probes the configured model via the same dispatch path the real run uses, with one tool defined and a prompt asking the model to call it. Pass emits `lyrik.model.tool_calls_supported` and continues. Fail-closed: the run aborts before scanner dispatch with the case named (`provider_rejected`, `tool_call_as_text`, `no_tool_call`, or `unparseable_arguments`) and recorded as `lyrik.model.tool_calls_unsupported`. A model that cannot emit structured tool calls produces zero findings, so the gate catches that at the door instead of after a wasted run.
- **Bundled Lyrik skill staged per run.** The Lyrik skill is written into the per-run state dir and self-signed with a one-shot keypair so the loader's signature gate accepts it. `/lyrik` in the dispatch prompt resolves to this staged copy regardless of operator state under `<data_dir>/skills/`. The same self-sign step runs on the walks staging path so `/<walk-name>` resolves under the gate too. New audit row: `lyrik.skill.staged`.
- **Citation-resolution gate (per-class structural).** For every emitted finding, the runner confirms the cited file exists and the cited line resolves. Beyond that, eight class-specific sub-gates fire on title/summary keywords, each running a scoped parser-free check on the cited line plus a +/-2-line window or routing to a named deferred tag when no honest check is available. Real checks (run a structural test): `literal_claim` requires a string or numeric literal in the window; `sql_injection_structural` requires a SQL keyword AND an interpolation marker AND NO parameter-binding marker (parameterized queries are the negative case and do not resolve); `broken_comparison_structural` requires `==`/`!=` against a credential-shaped identifier AND NO constant-time marker (`.ct_eq`, `subtle::ConstantTimeEq` are the negative case); `command_injection_structural` requires an `ident(` call shape on a non-comment line. Deferred tags (no structural check by design, existence-only resolution; the tag name carries the disclosure): `prompt_injection_deferred` (prompt injection is semantic, no parser-free signature); `race_condition_deferred` (TOCTOU is multi-line; single-line shared-state markers resolve on any concurrent code); `access_control_deferred` (an earlier draft's structural check resolved on any executable line and was dropped; promoted to a named tag for header honesty). Findings not matched by any class-keyword fall to `file_line_only`. Priority on multi-keyword findings: literal > prompt > sql > broken_comparison > race > access > command > file_line_only. Each finding's `citation_check` annotation names the gate that ran, and the canonical `findings.json` header reports counts split by all eight sub-gates so the unverified fraction is visible per class at a glance. The structural checks confirm the cited line *could plausibly host* the claimed bug class; they do not prove the bug is real or attacker-reachable. That matches Lyrik's grade-0.5 ceiling: real-and-reachable is the gate's promise, exploit verification is a separate workload. The three `*_deferred` tags carry the same existence-only caveat as `file_line_only.resolved` (file+line exist, nothing more); do not count them in the "verified" column. Tracked in [#143](https://github.com/gebruder/wirken/issues/143). Audit row: `lyrik.citation.checked` per finding.
...
- **JSON findings schema.** Schema-v1.1 with a reference validator (`wirken lyrik validate --path <path>`) and a SARIF report emitter (`wirken lyrik report --format sarif`) for SIEM and CI consumers.
