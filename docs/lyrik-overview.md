...
- **Citation-resolution gate (literal-claim scope).** For every emitted finding, the runner confirms the cited file exists and the cited line resolves. That is the only check most findings receive. When the finding's title or summary contains a hardcoded-literal claim word (`hardcoded`, `hard-coded`, `literal`), an additional check fires: the cited line or a small window around it must carry a string or numeric literal; a doc comment, a field declaration, or a type reference fails. Findings stay in the report either way; unresolved citations are flagged in-place under a per-finding `citation_check` block. The canonical `findings.json` header reports counts split by which check ran (`literal_claim.{resolved,unresolved}` vs `file_line_only.{resolved,unresolved}`), so a `file_line_only.resolved` row cannot be skimmed as a verified citation; it only means the line exists. Class-specific gates for non-literal claim shapes (missing access check, command injection, race condition, broken comparison) are tracked in [#143](https://github.com/gebruder/wirken/issues/143). Audit row: `lyrik.citation.checked` per finding.
