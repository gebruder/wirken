...
- **Citation-resolution gate (per-class structural).** For every emitted finding, the runner confirms the cited file exists and the cited line resolves. Beyond that, class-specific sub-gates fire on title/summary keywords, each running a scoped parser-free check on the cited line plus a +/-2-line window. `literal_claim` requires a string or numeric literal in the window (catches hardcoded-credential hallucinations). `sql_injection_structural` requires a SQL keyword AND an interpolation marker AND NO parameter-binding marker (catches concatenated query strings; parameterized queries are the negative case and do not resolve). `command_injection_structural` requires an `ident(` call shape on a non-comment line (catches command-injection claims that cite a doc comment). `prompt_injection_deferred` is routed by design to existence-only verification because prompt injection is a semantic property with no parser-free structural signature; the gate tag discloses this so a skimmer does not read a `prompt_injection_deferred` count as a structurally checked one. `prompt_injection_deferred.resolved` carries the same caveat as `file_line_only.resolved` (file+line exist, nothing more), and is in some ways weaker because the deferred sub-gate always resolves on existence by design. Anything not matched by a claim-class keyword falls to `file_line_only`. Priority on multi-keyword findings: literal > prompt > sql > command > file_line_only. Each finding's `citation_check` annotation names the gate that ran, and the canonical `findings.json` header reports counts split by sub-gate so the unverified fraction is visible at a glance. The structural checks confirm the cited line *could plausibly host* the claimed bug class; they do not prove the bug is real or attacker-reachable. That matches Lyrik's grade-0.5 ceiling: real-and-reachable is the gate's promise, exploit verification is a separate workload. Claim classes without a wired matcher (missing access check, race conditions, data leakage, broken comparison, etc.) fall to `file_line_only`. An earlier draft included an `access_control_structural` sub-gate; it was dropped because its check resolved on nearly any code line, padding the verified side of the header count without verifying the missing-check claim. A real access-control sub-gate needs a stronger discriminator. Scope expansion tracked in [#143](https://github.com/gebruder/wirken/issues/143). Audit row: `lyrik.citation.checked` per finding.
