...
- **Citation-resolution gate.** Each emitted finding is checked against the cited file and line. When the finding's title or summary contains a hardcoded-literal claim word (`hardcoded`, `hard-coded`, `literal`), the cited line or a small window around it must carry a string or numeric literal; a doc comment, a field declaration, or a type reference fails the check. Findings stay in the report; unresolved citations are flagged in-place under a per-finding `citation_check` block. Counts split by gate in the canonical `findings.json` header (`literal_claim.{resolved,unresolved}` vs `file_line_only.{resolved,unresolved}`), so a `file_line_only.resolved` count cannot be skimmed as a verified citation. The gate is narrow on purpose: it covers the literal-claim shape and existence-checks the rest. Audit row: `lyrik.citation.checked` per finding.
