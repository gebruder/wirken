...
**The scan is caught.** The detector is pattern matching over attacker-chosen
text, so a message can break it; when one did, the panic unwound the
per-connection task, and because the `message.inbound` row is written after
the scan that carries its verdict, the message that caused it was never
recorded at all. Catching the scan restores both: the call returns whatever
the detector does, so the row is written either way. A scanner panic emits
`message.threat_flagged` with `threat.scanner.panicked` and the reason,
