# Lyrik JSON output schema (post-1.0 design)

Status: forward-looking design. Lyrik 1.0 ships the minimal current
producer surface (`schema_version`, `run_id`, `produced_at`,
`findings[]`); see `docs/lyrik-json-schema.md` for the locked 1.0
spec. This document describes the larger funnel-disclosure surface
the producer is expected to grow into. Fields described here are
not yet emitted by lyrik and consumers must not pin against them.

This document was removed from the tree in commit 310a0c9 ("forward-
looking design moved to sibling") and restored here so the user-
review pressure-test it went through is not lost. New shape lands as
a future schema version, alongside producer changes; the move from
1.0 to that version is a separate slice.
