...
- **Default value when omitted — this is a posture decision, not a migration-cost decision.** Two framings:
  - **Default `false` (mirror OpenClaw, auto-fire allowed unless author explicitly disables).** Says: *Wirken's posture is that skills auto-fire unless authors explicitly opt out.* Optimises for skill-author convenience over operator safety. One-line lyrik opt-in covers today's only known case.
  - **Default `true` (explicit-only unless author explicitly enables auto-fire).** Says: *Wirken's posture is that auto-invocation requires explicit author opt-in.* Matches Wirken's broader stance on capability minimisation (vault, sandbox, mode `exec-only` defaults). Requires sweeping the 16 bundled skills to add `disable-model-invocation: false` for the ones that want auto-fire (git, github, web-fetch, calculator, csv-tools, file-search, json-tools, notes, process-manager, ssh, system-info, tmux, weather, disk-usage, docker — most of them).
  - **The migration cost (sweep 16 skills) is one-time. The posture choice is permanent.** Default-to-`true` is the consistent posture for a security-conscious orchestration platform; do not let migration ease select default-to-`false` by default.
- **How does the harness enforce it.** Two options:
  - **Load-time exclusion.** At skill-load, the harness records explicit-only skills and excludes them from the system prompt's auto-pickable set. Their bodies still load (so the agent knows about them), but the agent does not propose them in response to generic messages.
...
- **What "explicit invocation" looks like — convention question that ships with the primitive, not after.** OpenClaw supports both shapes across the 449 skills (slash-command-style skills and skill-name-mention skills). Wirken probably wants both too, but the convention has to be documented before shipping or operator behavior will be inconsistent across skills.
  - **Slash commands** (`/lyrik audit ./crates/vault`) signal *"this is a command, I'm running a tool."* Different mental model than skill-name mentions; failure mode is operator typos / wrong arg syntax.
  - **Skill-name mentions** (`use lyrik to audit the vault crate`) signal *"I'm asking the agent to use a skill."* More natural-language, less command-shell. Failure mode is fuzzy matching — what counts as an explicit mention vs a generic description match.
  - **Both shapes documented and supported** is the likely answer; pick the syntax for each before shipping. Document in `docs/lyrik.md` (and equivalent docs for any other explicit-only skill) which form a given skill expects, with examples. Inconsistent conventions across skills is the failure mode to avoid.
